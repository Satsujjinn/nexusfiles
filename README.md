...
On Linux the `libxlsxwriter` C library is built automatically via Swift Package Manager. The share extension uses UIKit, which is unavailable on Linux, so building the `NexusFilesShareExtension` target will fail. Use macOS with Xcode for the complete experience.
